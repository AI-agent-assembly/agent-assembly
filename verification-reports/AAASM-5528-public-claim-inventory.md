...
| The loader daemon binary is absent from the GitHub Release assets, the Homebrew tap, Docker/GHCR images and the curl installer — but `aa-ebpf` carries a `[[bin]] name = "aa-ebpf-loaderd"`, has no `publish = false`, and is untouched by the publish strip, so the crate **is published to crates.io** and the daemon is reachable via `cargo install`/`cargo build` on Linux; verified against the downloaded `aa-ebpf-0.0.1-rc.6.crate` tarball, which contains `src/bin/loaderd.rs` (AAASM-5652: `docs/src/security-model.md` published the absolute "cannot be activated from a released build", which this row corrects — added here so the evidence trail behind that page names the same channels the page now does). That path is undocumented; whether it counts as a supported install method is AAASM-5640's call, not yet made. Reachability without the toolchain still fails closed: with no nightly toolchain plus `rust-src` and `bpf-linker`, `build.rs` writes empty stub files, and `verify_bytecode` treats an empty digest as unverifiable and refuses to load it | `aa-ebpf/Cargo.toml:51-53` (bin target); absence of `publish` key verified in `aa-ebpf/Cargo.toml`; `aa-ebpf/build.rs:38-40,133-134,155-156`; `aa-ebpf/src/integrity.rs:57,60-71` |
