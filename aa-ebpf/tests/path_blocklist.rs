// Integration test: verify that accessing a blocklisted path generates
// a policy violation event (flags bit 0 = 1).
//
// Gated on:
...
//   sudo env "PATH=$PATH" cargo test -p aa-ebpf --features integration-test --test path_blocklist -- --nocapture
...
/// Load BPF programs, add /etc/passwd to the path blocklist, open it,
...
async fn blocklisted_path_triggers_sensitive_flag() {
    ...
    .await
    .expect("timed out waiting for blocklist event")
    .expect("no sensitive event received");
