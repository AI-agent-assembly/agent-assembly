...
| v0.0.1-rc.7 | v0.0.1-rc.7 (PyPI `0.0.1rc7`) ✓ | v0.0.1-rc.7 ✓ | v0.0.1-rc.7 ✓ | protocol/v1 |
...
> **Note (v0.0.1-rc.7):** seventh release candidate (patch on the `rc` channel) — the largest cut since rc.6, spanning four programmes. A new backend-neutral execution-isolation layer for `aasm run` (Linux `sandlock`, AASM-native seccomp, and macOS VM backends, each reached only when measured-supported); trusted upstream proxy chaining (Core ADR 036) with per-launch dedicated proxy instances and provider-aware credential injection; sensitive-data analytics/drill-down plus audit `decision_id` correlation; and a batch of gateway/proxy/CLI security fixes (non-loopback bind refusals on `aa-runtime`/`aa-cli`/`aa-api`/`aa-gateway`, audit backpressure-vs-tamper distinction, credential-exposure fixes in isolation confinement argv and CA cert persistence). Full detail in `CHANGELOG.md`'s `[Unreleased]` section (now folded into this tag). No wire-protocol change. `aa-runtime` v0.0.1-rc.7 pairs with python-sdk `0.0.1rc7`, node-sdk `v0.0.1-rc.7`, and go-sdk `v0.0.1-rc.7`, all `protocol/v1`-compatible. SDK `rc.7` cuts follow this tag's `release.yml` fan-out (per the `aa-ffi-pin` SDK-coordination SOP).
...
| AAASM-5499 | Ratified the public `aasm integrations` outcome contract and implemented it for `repair` and `remove`. The `RepairReport` and `RemoveReport` JSON/YAML documents gain an **`outcome`** field carrying `changed`, `unchanged` or `null`; `ChangeOutcome` in `aa-cli/src/commands/integrations/exit.rs` is the vocabulary, and `refused`/`failed` are named on stderr on the non-zero paths. **No exit code was added or changed** — the eleven-value table from AAASM-5280/5628 is untouched, and a legitimate no-op still exits `0`. | **Additive, no break.** New JSON keys only: `outcome` on the `repair` and `remove` reports. `nothing_to_repair` (AAASM-5455) and `plan_id` (AAASM-5629) keep their existing shapes and values; they are now set by the same constructor call as `outcome` so the two cannot disagree. **Behavior delta:** `repair --dry-run` against an *installed, undrifted* tool now reports the no-op the way the phase short-circuit already did — first line marked `unchanged (nothing to repair)` and `nothing_to_repair` set instead of `null`. Its exit code is unchanged (`0`), as is `repair --dry-run` with drift (`5`). One stderr line (`outcome: <token> (exit N name)`) is added to every non-zero exit; stdout on those paths stays empty, preserving AAASM-5628's "a refused command leaves a harness no result to record". `install` is **not** covered by this ticket: `ApplyView` carries no mutation flag, so adding it there is a DI-API wire change — since done in AAASM-5674 below, which supersedes this sentence. No public Rust API, protocol, or ABI change. |
| AAASM-5674 | Raised the Developer Integration API to **v5** (`DI_API_MAX_SUPPORTED = 5`, `DI_API_APPLY_OUTCOME_SINCE = 5`): `ApplyView` gained an optional `ApplyOutcomeView` message (`proto/devint.proto`, field 7) stating whether the apply modified the host — `CHANGED`, `UNCHANGED`, `FAILED`, `UNSUPPORTED`, or `UNSPECIFIED` for "no outcome was stated". Carried as a message holding an enum rather than a `bool`, because a proto3 `bool` has no presence and its default `false` would decode as `unchanged` — a success claim fabricated out of a field no peer sent. v5 adds **no verb**, so a v1–v4 peer stays `SUPPORTED` with the whole verb space. `aasm integrations install` now reports the ratified `outcome` (AAASM-5499); this **supersedes** the "`install` is **not** covered" note in the AAASM-5499 row above. | **Additive on the wire, source-breaking in one Rust signature.** Protocol: a v1–v4 client is unaffected and is never sent the new field; a v5 client against a v1–v4 runtime reads the outcome as *not reported*, never as `unchanged`. New JSON keys only: **`outcome`** and **`outcome_unknown`** on the `install` report. `outcome` is `changed`, `unchanged` or `null`; `null` means the runtime did not state one and is **not** `unchanged` — branch on it explicitly. `install`'s exit codes are unchanged, including `0` in the `null` case, because the apply itself succeeded. **Rust source break:** `aa_runtime::devint::IntegrationLifecycle::apply` now returns `AppliedIntegration { receipt, mutation }` instead of `IntegrationReceipt`; any out-of-tree implementor or caller of that trait must be updated (`.receipt` recovers the previous value). No ABI change, no CLI removal, no exit-code change. |
